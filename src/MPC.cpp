...
size_t N = 10;
// no reason to be faster than overall latency
double dt = 0.130;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N; t++) {
      fg[0] += 2*CppAD::pow(vars[cte_start + t], 4);
    ...
    for (int t = 0; t < N - 1; t++) {
      ...
      fg[0] += 0.003*CppAD::pow(vars[delta_start + t] * vars[v_start + t] * vars[v_start + t], 2);
    }
    ...
    for (int t = 1; t < N; t++) {
      ...
      fg[1 + v_start + t] = v1 - (v0 + a0 * dt * 5.);   // decel is very fast
      fg[1 + cte_start + t] =
